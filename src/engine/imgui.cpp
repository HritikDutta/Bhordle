...
static constexpr s32 maxQuadCount = 20000;
...
static void InitWhiteTexture(int width, int height)
{
    ...
    uidata.whiteTexture.LoadPixels("whiteTexture", pixels, width, height, 4, settings);
...
static void InitBatches()
{
    ...
    Vertex* batchBuffer = (Vertex*) PlatformAllocate(2 * batchSize * sizeof(Vertex));
    ...
    {   // Init Quad Batch
        ...
        uidata.quadBatch.elemVerticesBuffer = batchBuffer;
...
void Shutdown()
{
    ...
    PlatformFree(uidata.quadBatch.elemVerticesBuffer);  // Since both batches are allocated together
...
{
    // Since these will be used for all images
    static Rect rect;
    static Vector4 texCoords { 0.0f, 1.0f, 1.0f, 0.0f };
